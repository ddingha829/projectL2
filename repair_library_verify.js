    ...
    for (const review of reviews) {
        ...
        if (movieData.poster_path) {
            ...
            const { data, error: updateError } = await supabase
                ...
                .eq('id', review.id)
                .select(); // Check what was updated
            
            if (updateError) {
                console.error(`Error updating ${review.subject}:`, updateError);
            } else {
                console.log(`Updated ${review.subject}. New data:`, JSON.stringify(data, null, 2));
            }
        }
    }
}
