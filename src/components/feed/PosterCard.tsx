...
export default function PosterCard({ 
  ...
  return (
    <article className={`${styles.card} ${aspectRatio === 'card45' ? styles.ratioCard45 : ''} ${aspectRatio === 'mag53' ? styles.ratioMag53 : ''} ${isOneCol ? styles.oneColCard : ''} ${isDense ? styles.denseCard : ''} ${isMinimal ? styles.minimalCard : ''}`}>
      ...
      <div className={styles.contentArea}>
        <h3 className={styles.title}>
          <Link href={`/post/${id}`} className={styles.stretchedLink} style={title.length > 40 ? { fontSize: '0.82em' } : title.length > 25 ? { fontSize: '0.9em' } : {}}>
            {title}
          </Link>
        </h3>
